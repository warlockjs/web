...
const { capturedStreams } = vi.hoisted(() => ({
  capturedStreams: [] as Array<
    PipeableStream & { abort: ReturnType<typeof import("vitest").vi.fn> }
  >,
}));
...
describe("streamed document parity — streamed bytes equal the buffered renderToString bytes", () => {
   ...
   */
  it("puts byte-identical bytes on the wire to what render-page.ts's own buffered .html describes for the SAME render", async () => {
...
describe("first byte does not leave before loaders/validation/middleware settle", () => {
  it("writes zero bytes and calls no writeHead while a page loader is still pending, then flushes once it resolves", async () => {
    let release!: () => void;
...
describe("a client disconnect mid-stream aborts the React render", () => {
  ...
  it("destroying the raw response before it finishes calls pipeableStream.abort()", async () => {
    const page = {
