...
describe("streamed defer() — a resolved deferred value", () => {
  it("streams the shell first, then the chunk script carrying the request's nonce, then ends", async () => {
  ...
  it("ends the response only after the last deferred key has settled", async () => {
...
describe("streamed defer() — a rejected deferred value", () => {
  it("emits an ok:false chunk with no stack in production, and reports to the server error sink", async () => {
    ...
    const settlement = decodedDeferSettlement(wireHtml, "reviews") as {
      ok: boolean;
      error: { name: string; message: string; stack?: string; errorCode?: string };
    };
    expect(settlement.ok).toBe(false);
    expect(settlement.error.name).toBe("Error");
    // The real message never reaches the browser in production — only an
    // explicit `PublicPageError` may expose its own message. An opaque
    // `errorCode` joins this chunk to the server's own report line below.
    expect(settlement.error.message).toBe("An unexpected error occurred.");
    expect(settlement.error.message).not.toContain("reviews service is down");
    ...
    expect(errorSink.mock.calls[0]?.[0]).toContain(settlement.error.errorCode);
  }, 20_000);
});

describe("streamed defer() — a deferred value that never settles", () => {
  it("settles the chunk with DeferTimeoutError once web.streaming.deferTimeout elapses", async () => {
    setConfig("web", { streaming: { deferTimeout: 25 } });
    const errorSink = vi.spyOn(console, "error").mockImplementation(() => undefined);

    const page = {
      loader: async () => defer({ greeting: "hi", reviews: new Promise<never>(() => undefined) }),
      default: ({ data }: { data: { greeting: string } }) =>
        createElement("main", null, data.greeting),
    };
    ...
    expect(
      errorSink.mock.calls.some((call) =>
        String(call[0]).includes('deferred value "reviews" timed out'),
      ),
    ).toBe(true);
  }, 20_000);
});
...
describe("streamed defer() — chunk escaping", () => {
  it("a deferred value containing </script> markup cannot break out of its own <script> chunk", async () => {
    const hostile = "</script><script>alert(1)</script>";

    const page = {
      loader: async () => defer({ greeting: "hi", reviews: Promise.resolve({ comment: hostile }) }),
      default: ({ data }: { data: { greeting: string } }) =>
        createElement("main", null, data.greeting),
    };
