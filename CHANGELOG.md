...
- `web.loaderTimeout` bounds the non-deferred loader chain of one page request (app → layout → page loaders, plus `validation`) — default 15000ms, `0` disables it. A loader that never settles now aborts the request's `ctx.signal` and fails with `PageLoaderTimeoutError`, rendering your `error.page.tsx` with status 504 (the same 504 on the data wire) instead of holding the request and socket open forever. `defer()`-ed values and the render itself are unaffected — they keep their own existing bounds.
- The production client build now splits React, ReactDOM, the scheduler and `react/jsx-runtime` into their own `vendor-react-<hash>.js` chunk, separate from the `hydration-<hash>.js` entry (which keeps only the Warlock client runtime) and from every per-page chunk. React rarely changes between app deploys, so it now keeps a stable, cacheable hash across deploys that only touch app code, instead of re-downloading on every one. The document now also emits `<link rel="modulepreload">` for the entry's own statically imported chunks (the vendor chunk, most of all), fetched in parallel with `<Scripts/>`'s `<script type="module">` rather than discovered only after the entry parses far enough to see its own `import`.
- Error reporting hooks. `web.errors.report(error, context)` receives every server-side render, loader, defer, error-page and request-handler failure, with `kind`, `phase`, route name and template, pathname (never the query string), method, status and request id. It runs fire-and-forget and is never awaited on the response path. A throwing reporter is logged once and never recurses, the same error object is reported once, and graceful shutdown waits up to 2s for pending reports. At most 10 reporter calls run at once, with up to 100 more queued behind them; past that the oldest not-yet-started report is dropped and counted, so a slow or stuck reporter can never grow the number of active calls without bound. On the client, `onClientError(callback)` receives uncaught window errors, unhandled rejections, React hydration errors and boundary catches, including an async reporter's own rejection, which is caught and logged once instead of escaping as a new unhandled rejection. The `console.error` output is unchanged either way.
- Hydration now runs inside `startTransition`, so React time-slices it instead of one non-yielding task. On the blog, mobile Total Blocking Time (4x CPU, median of 5) dropped about 20% (469 to 366 ms on `/`, 512 to 425 ms on a post).
...
- Authenticated requests never get a public `Cache-Control`. A request that used authenticated state (including through `authMiddleware` on a page or layout) is sent `private, no-store`, even on a route that opted into `config.cache.public`.
